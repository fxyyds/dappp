...
const trendChart = ref<EchartsUIType>();
const categoryChart = ref<EchartsUIType>();
const provinceChart = ref<EchartsUIType>();
const channelChart = ref<EchartsUIType>();

const { renderEcharts: renderTrend } = useEcharts(trendChart);
const { renderEcharts: renderCategory } = useEcharts(categoryChart);
const { renderEcharts: renderProvince } = useEcharts(provinceChart);
const { renderEcharts: renderChannel } = useEcharts(channelChart);
...
function drawTrend(points: SalesApi.TrendPoint[]) {
  renderTrend({
    ...
    series: [
      {
        ...
        data: points.map((p) => p.saleAmount),
...
function drawRankBar(
  render: (option: Record<string, any>) => unknown,
  items: SalesApi.DimRankItem[],
) {
  ...
  render({
    ...
    series: [
      {
        barMaxWidth: 14,
        data: sorted.map((item) => item.saleAmount),
        itemStyle: {
...
function drawChannel(items: SalesApi.DimRankItem[]) {
  renderChannel({
    backgroundColor: 'transparent',
    color: COLORS,
    legend: {
    ...
    series: [
      {
        data: items.map((item) => ({ name: item.name, value: item.saleAmount })),
...
async function fetchData() {
  try {
    ...
    drawTrend(trend ?? []);
    drawRankBar(renderCategory, category ?? []);
    drawRankBar(renderProvince, province ?? []);
    drawChannel(channel ?? []);
  ...
  <div class="sales-screen">
    ...
    <section class="chart-row">
      <div class="panel panel-wide">
        ...
        <div class="panel-body">
          <EchartsUI ref="trendChart" height="100%" width="100%" />
        </div>
      </div>
      <div class="panel">
        <div class="panel-title">类目销售排行</div>
        <div class="panel-body">
          <EchartsUI ref="categoryChart" height="100%" width="100%" />
        </div>
      </div>
    </section>
    ...
    <section class="chart-row">
      <div class="panel">
        ...
        <div class="panel-body">
          <EchartsUI ref="provinceChart" height="100%" width="100%" />
        </div>
      </div>
      <div class="panel panel-wide">
      ...
      <div class="panel">
        ...
        <div class="panel-body">
          <EchartsUI ref="channelChart" height="100%" width="100%" />
        </div>
      </div>
    </section>
  </div>
